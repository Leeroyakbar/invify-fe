...
import { fadeSoft, scaleFade } from "../../../../motions/templateMotions"

export default function BrideSection({ data }: { data: Invitation }) {
  return (
    <section id="bride" className="h-screen snap-start relative overflow-hidden text-white">
      {/* BACKGROUND IMAGE */}
      <div
        className="absolute inset-0 bg-cover bg-center"
        style={{
          backgroundImage: "url('/classic-noir/bride-2.jpg')",
        }}
      />
      ...
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: false, amount: 0.4 }} variants={fadeSoft} className="relative z-10 h-full flex flex-col justify-end px-8 pb-16">
        <p className="text-[11px] tracking-[0.3em] mb-3 opacity-80">THE BRIDE</p>

        <h1 className="font-serif text-4xl md:text-5xl mb-2">{data.brideFullName}</h1>
        <div className="w-22 h-px bg-white/40 my-4" />

        <p className="italic text-sm opacity-80 mb-4">The Daughter of</p>

        <p className="text-sm leading-relaxed max-w-md opacity-90">
          {data.brideFather} <br />& {data.brideMother}
        </p>

        {/* OPTIONAL TAG */}
        <div className="mt-6 ">
          <a href={`https://instagram.com/${data.brideInstagram}`} target="_blank">
            <span className="inline-block px-4 py-1 text-xs border border-white/40 rounded-full items-center">@ {data.brideInstagram}</span>
          </a>
        </div>
      </motion.div>
    </section>
