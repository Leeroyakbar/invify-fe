...
export default function GroomSection({ data }: { data: Invitation }) {
  return (
    <section className="h-screen snap-start bg-[#F9F8F4] relative flex flex-col overflow-hidden">
      ...
      <div className="absolute top-20 right-[-10%] text-[20rem] font-serif italic text-stone-200/40 leading-none pointer-events-none select-none">G</div>

      {/* Main Content Area */}
      <div className="flex-1 flex flex-col md:flex-row items-center justify-center p-8 md:p-20 z-10 gap-8">
        ...
        <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} transition={{ duration: 1.5, ease: [0.22, 1, 0.36, 1] }} className="w-full md:w-[45%] h-[50vh] md:h-[70vh] relative">
          ...
          <img src={data.groomPhoto} className="w-full h-full object-cover grayscale sepia-[0.1] shadow-2xl" alt="The Groom" />
        </motion.div>
        ...
        <div className="w-full md:w-[55%] text-center md:text-left">
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} className="space-y-2">
            <p className="text-[10px] tracking-[0.2em] text-stone-400 uppercase italic">The beloved Son of</p>
            <p className="text-xs md:text-sm tracking-[0.3em] text-stone-600 uppercase font-light">
              Mr. {data.groomFather} <span className="mx-2 opacity-30">&</span> Mrs. {data.groomMother}
            </p>
          </motion.div>
        </div>
      </div>
