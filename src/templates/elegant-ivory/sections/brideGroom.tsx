...
export default function BrideGroomSection({ data }: Props) {
  return (
    <section className="bg-[#F8F6F2] px-6 py-24 text-center">
      ...
      <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ duration: 1.0, delay: 0.1 }} viewport={{ once: false }} className="mx-auto mt-4 max-w-md font-inter text-sm text-[#6B7280]">
      ...
      <div className="mt-20 flex flex-col items-center gap-10">
        ...
        {/* Groom */}
        <PersonBlock image={data.groomPhoto} fullName={data.groomFullName} bride={false} father={data.groomFather} mother={data.groomMother} delay={0.2} />
      </div>
    </section>
  )
}
