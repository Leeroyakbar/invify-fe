...
export default function CountdownSection({ data }: CountdownProps) {
  ...
  return (
    <section className="bg-[#F8F6F2] px-6 py-20 text-center">
      ...
      <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} transition={{ delay: 0.15 }} viewport={{ once: true }} className="mx-auto mt-4 max-w-md font-inter text-sm text-[#6B7280]">
        ...
        {items.map((item, i) => (
          <div key={item.label} className="flex items-center gap-6">
            <div>
              ...
              <div className="mt-1 font-inter text-[10px] uppercase tracking-widest text-[#8FA8A1]">{item.label}</div>
            </div>
      ...
      {/* CTA */}
      <motion.button whileTap={{ scale: 0.96 }} className="mx-auto mt-14 rounded-full border border-[#C8A97E] px-8 py-3 font-inter text-sm text-[#2F3E46] transition hover:bg-[#C8A97E]/10">
        Save the Date
      </motion.button>
    </section>
  )
